...
vec3 getBloom( in vec2 uv, in sampler2D t, in float offset ) {
    vec3 bloom = vec3(0.0);
    for( int i = 0; i < 8; i++ ) {
        vec4 sample0 = texture2D( t, vec2(uv.x + ( offset * float( i ) ), uv.y ) );
        vec4 sample1 = texture2D( t, vec2(uv.x - ( offset * float( i ) ), uv.y ) );
        vec4 sample2 = texture2D( t, vec2(uv.x, uv.y + ( offset * float( i ) ) ) );
        vec4 sample3 = texture2D( t, vec2(uv.x, uv.y - ( offset * float( i ) ) ) );
        bloom += sample0.rgb * ( float( 8 - i ) * 0.6 );
        bloom += sample1.rgb * ( float( 8 - i ) * 0.6 );
        bloom += sample2.rgb * ( float( 8 - i ) * 0.6 );
        bloom += sample3.rgb * ( float( 8 - i ) * 0.6 );
    }
    return bloom;
}

vec3 getShadow( in vec2 uv, in sampler2D t, in float offset ) {
    vec3 bloom = vec3(0.0);
    for( int i = 0; i < 8; i++ ) {
        vec4 sample0 = texture2D( t, vec2(uv.x + ( offset * float( i ) ), uv.y ) );
        vec4 sample1 = texture2D( t, vec2(uv.x - ( offset * float( i ) ), uv.y ) );
        vec4 sample2 = texture2D( t, vec2(uv.x, uv.y + ( offset * float( i ) ) ) );
        vec4 sample3 = texture2D( t, vec2(uv.x, uv.y - ( offset * float( i ) ) ) );
        bloom += sample0.rgb * ( float( 8 - i ) * 2.0 );
        bloom += sample1.rgb * ( float( 8 - i ) * 2.0 );
        bloom += sample2.rgb * ( float( 8 - i ) * 2.0 );
        bloom += sample3.rgb * ( float( 8 - i ) * 2.0 );
    }
    return vec3( 1.0 - bloom );
}


vec3 texColor ( in vec2 uv, in sampler2D t, in float offset, in float amount, in float h_shit, in float v_shit, in float tmult ) {
	float x = sin( tmult * time + uv.y * 21.0 ) * sin( tmult * time + uv.y * 29.0 ) * sin( 0.3 + 0.33 * time + uv.y * 31.0 ) * h_shit;
    float y = sin( tmult * time + uv.x * 21.0 ) * sin( tmult * time + uv.x * 29.0 ) * sin( 0.3 + 0.33 * time + uv.x * 31.0 ) * v_shit;
    vec2 uvd = vec2( x + uv.x, y + uv.y );
    vec4 texColor = texture2D( t, uvd );
    vec3 bloom = getBloom( uvd, t, offset * 3.0 ) * amount;
    vec3 shadow = getShadow( uv, t, offset );
    return ( shadow + texColor.rgb / bloom );
}
...
void main ( void ) {
    ...
    vec3 colA = color.rgb;
    vec3 colB = tx;
    vec3 txColor = clamp( mix( colA, colB, 0.05 ), 0.0, 1.0 );
    txColor = vec3( 0.0, txColor.g, 0.0 ) * 2.0;
    vec3 tColor = color.rgb * ( 1.0 - txColor );
    vec4 t = texture2D( iChannel0, tUv );
    vec3 finalColor = t.rgb + tColor * 0.4;
